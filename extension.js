...
import { PopupMenuItem, PopupSeparatorMenuItem, PopupSwitchMenuItem } from 'resource:///org/gnome/shell/ui/popupMenu.js';
...
class GEWallpaperIndicator extends Button {
    _init(extension) {
        super._init(/*menuAlignment*/ 0, /*nameText*/ IndicatorName, /*dontCreateMenu*/ false);

        this._extension = extension;
        this._settings = extension.getSettings();

        let gicon = Gio.icon_new_for_string(this._extension.dir.get_child('icons').get_path() + "/" + this._settings.get_string('icon') + "-symbolic.svg");
        ...
        this.httpSession = Utils.initSoup(this._extension.metadata.version); // create libSoup session for http requests
        ...
        this.refreshDueItem = new PopupMenuItem(_("<No refresh scheduled>"));
        this.descriptionItem = new PopupMenuItem(_("Text Location"));
        this.locationItem = new PopupMenuItem(_("Geo Location"));
        this.extLinkItem = new PopupMenuItem(_("External Link"));
        this.copyrightItem = new PopupMenuItem(_("Copyright"));
        this.dwallpaperItem = new PopupMenuItem(_("Set background image now"));
        this.swallpaperItem = new PopupMenuItem(_("Set lockscreen image now"));
        this.refreshItem = new PopupMenuItem(_("Refresh Now"));
        this.settingsItem = new PopupMenuItem(_("Extension settings"));
        ...
        this.lockscreenToggle = this._newMenuSwitch(_("Set lockscreen image"), "set-lock-screen", this._settings.get_boolean('set-lock-screen'), false);
        ...
        this.menu.addMenuItem(new PopupSeparatorMenuItem());
        this.menu.addMenuItem(new PopupMenuItem(_("On refresh:"), { reactive: false }));
    ...
    _setMenuConnections() {
        ...
        this.connect('button-press-event', this._updateMenu.bind(this));
    }

    _open_link() {
        spawn(["xdg-open", this.link]);
    }
    ...
    _createNotification() {
        ...
        let source = new Source('Google Earth Wallpaper', 'preferences-desktop-wallpaper-symbolic');
        main.messageTray.add(source);
        let msg = 'Google Earth Wallpaper';
        let details = this.explanation + '\n' + Utils.friendly_coordinates(this.lat, this.lon) + '\n' + this.copyright;
        let notification = new Notification(source, msg, details);
    ...
    _newMenuSwitch(string, dconf_key, initialValue, writable) {
        ...
        let widget = new PopupSwitchMenuItem(string,
            initialValue);
        if (!writable) {
    ...
    _http_process_message(httpSession, message) {
        try {
            ...
            if (Soup.MAJOR_VERSION >= 3) {
                data = Utils.Bytes.decode(this.httpSession.send_and_read_finish(message).get_data());
            }
            else {
    ...
    _setIcon() {
        Utils.validate_icon(this._settings, this._extension.dir);
        let icon_name = this._settings.get_string('icon');
        let gicon = Gio.icon_new_for_string(this._extension.dir.get_child('icons').get_path() + "/" + Utils.icon_list_filename[Utils.icon_list.indexOf(icon_name)] + ".svg");
        this.icon = new St.Icon({ gicon: gicon, style_class: 'system-status-icon' });
...
export default class GoogleEarthWallpaper extends Extension {
    enable() {
        if (!globalThis.GoogleEarthWallpaperState) globalThis.GoogleEarthWallpaperState = {
            gettext : _,
            PACKAGE_VERSION
        };
        googleearthWallpaperIndicator = new GEWallpaperIndicator(this);
    ...
    disable() {
        ...
        delete globalThis.GoogleEarthWallpaperState;
    }
}